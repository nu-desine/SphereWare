...
/*
 * The encoder has to be read at a reasonable rate. The previous state is 
 * shifted up. Fast turns are detected by looking at the upper byte in the 
 * state history and seeing if it had has had time to return to 0b11 
 * (unturned state) 4 reads in a row (== 0xFF).
 */
void _Encoder_Read(bool number)
{
